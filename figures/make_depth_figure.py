...
def main() -> None:
    ...
    for key, (x, y, label, on) in NODES.items():
        ...
        ax.text(
            ...
            color="#111",
            zorder=4,
            fontweight="bold" if key == "dr" else "normal",
        )
    ...
    ax.plot([0.36, 2.64], [1.44, 1.44], linewidth=0.8, color=CHAIN, solid_capstyle="butt", zorder=2)
    for x in (0.36, 2.64):
        ax.plot([x, x], [1.41, 1.47], linewidth=0.8, color=CHAIN, zorder=2)
    ax.text(
        1.5,
        1.50,
        "longest chain of consecutive reactions: 3",
        ha="center",
        va="bottom",
        fontsize=6.6,
        color=CHAIN,
    ...
    ax.text(
        1.5,
        -0.34,
        "whole derivation: 4 reactions, counting r1–r4 once each",
        ha="center",
        va="center",
        fontsize=6.6,
        color="#111",
    ...
    ax.set_aspect("equal")
    ax.axis("off")
    save(fig, str(OUT / "Figure_S2_depth_vs_count"))
    print("wrote figures/output/Figure_S2_depth_vs_count.png")
