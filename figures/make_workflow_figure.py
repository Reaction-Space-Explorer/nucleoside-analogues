...
Boxes name the stage and, beneath, the module or data that carries it out, so
the figure and the code cannot drift apart without one of them looking wrong.
"""
...
a2 = box(
    ax, xr, 0.945, W, H, "Chemical reaction network\nMØD graph grammar, seeds expanded G0–G6", "in"
)
...
d = box(
    ax, 0.5, 0.405, C, H, "Reaction free energies\neQuilibrator component contribution: ΔrG′° ± σ"
)
e = box(
    ax, 0.5, 0.225, C, H, "Three-way spontaneity call\nspontaneous / non-spontaneous / undetermined"
)
f = box(
    ax,
    0.5,
    ...
    C,
    H,
    "Minimum-weight hyperpath (Knuth)\nsteps to each target, and minimum-cost route count",
    "out",
)
